...
claim_rate_country_input_tbl <- tribble(
    ~country_code, ~year, ~rate_mu, ~rate_sd, ~rep_mu, ~rep_sd 
   ,        "DEU",  2000,     0.25,     0.05,    2.30,    1.00
   ,        "DEU",  2010,     0.30,     0.05,    2.50,    1.00
   ,        "DEU",  2018,     0.20,     0.05,    2.30,    0.80
   ,        "FRA",  2000,     0.20,     0.05,    2.50,    1.00
   ,        "FRA",  2018,     0.20,     0.05,    2.30,    1.00
   ,        "GBR",  2000,     0.20,     0.05,    2.70,    1.00
   ,        "GBR",  2010,     0.20,     0.05,    2.50,    1.00
   ,        "GBR",  2018,     0.15,     0.05,    2.30,    1.00
   ,        "IRL",  2000,     0.25,     0.05,    2.30,    1.00
   ,        "IRL",  2010,     0.20,     0.05,    2.50,    1.30
   ,        "IRL",  2018,     0.30,     0.05,    2.10,    0.80
   ,        "ESP",  2000,     0.20,     0.05,    2.30,    1.00
   ,        "ESP",  2018,     0.20,     0.05,    2.30,    1.00
   ,        "ITA",  2000,     0.20,     0.05,    2.30,    1.00
   ,        "ITA",  2018,     0.20,     0.05,    2.30,    1.00
   ,        "NLD",  2000,     0.20,     0.05,    2.30,    1.00
   ,        "NLD",  2018,     0.20,     0.05,    2.00,    1.00
   ,        "BEL",  2000,     0.20,     0.05,    2.30,    1.00
   ,        "BEL",  2018,     0.20,     0.05,    2.00,    1.00
   ,        "AUT",  2000,     0.30,     0.05,    2.30,    1.00
   ,        "AUT",  2018,     0.20,     0.05,    2.00,    0.90
   ,        "POL",  2000,     0.35,     0.05,    2.50,    1.00
   ,        "POL",  2018,     0.25,     0.05,    2.30,    1.00
   ,        "PRT",  2000,     0.20,     0.05,    2.50,    1.00
   ,        "PRT",  2018,     0.20,     0.05,    2.30,    1.00
   ,        "SWE",  2000,     0.20,     0.05,    2.30,    1.00
   ,        "SWE",  2018,     0.20,     0.05,    2.00,    1.00
   ,        "DNK",  2000,     0.20,     0.05,    2.50,    1.00
   ,        "DNK",  2018,     0.20,     0.05,    2.30,    1.00
)
...
claim_size_pd_country_input_tbl <- tribble(
    ~country_code, ~year, ~size_mu, ~size_sd, ~size_rt, ~min_claim_size
   ,        "DEU",  2000,     2000,     0.10,    0.001,           50.00
   ,        "DEU",  2018,     3000,     0.10,    0.002,          100.00
   ,        "FRA",  2000,     2000,     0.10,    0.001,           50.00
   ,        "FRA",  2018,     3000,     0.10,    0.001,          100.00
   ,        "GBR",  2000,     2000,     0.10,    0.001,           50.00
   ,        "GBR",  2018,     4000,     0.10,    0.001,          100.00
   ,        "IRL",  2000,     2000,     0.10,    0.001,           50.00
   ,        "IRL",  2010,     3000,     0.10,    0.001,          100.00
   ,        "IRL",  2018,     3500,     0.10,    0.001,           75.00
   ,        "ESP",  2000,     1500,     0.10,    0.001,           50.00
   ,        "ESP",  2018,     2500,     0.10,    0.001,          100.00
   ,        "ITA",  2000,     2000,     0.10,    0.001,           50.00
   ,        "ITA",  2018,     3000,     0.10,    0.001,           75.00
   ,        "NLD",  2000,     1500,     0.10,    0.001,           50.00
   ,        "NLD",  2018,     2000,     0.10,    0.001,          100.00
   ,        "BEL",  2000,     1000,     0.10,    0.001,           50.00
   ,        "BEL",  2018,     2000,     0.10,    0.001,          100.00
   ,        "AUT",  2000,     1000,     0.10,    0.001,           50.00
   ,        "AUT",  2018,     2000,     0.10,    0.001,          100.00
   ,        "POL",  2000,      500,     0.10,    0.001,           25.00
   ,        "POL",  2018,     1000,     0.10,    0.001,           50.00
   ,        "PRT",  2000,     1000,     0.10,    0.001,           30.00
   ,        "PRT",  2018,     1500,     0.10,    0.001,           80.00
   ,        "SWE",  2000,     2000,     0.10,    0.001,           50.00
   ,        "SWE",  2018,     3000,     0.10,    0.001,          100.00
   ,        "DNK",  2000,     1000,     0.10,    0.001,           50.00
   ,        "DNK",  2018,     2500,     0.10,    0.001,          100.00
)
...
country_claim_params_tbl <- claim_size_bi_country_input_tbl %>%
    ...
    summarise(pd_mu = list(interp_yearly_params(year, 'size_mu_pd', size_mu_pd))
             ...
             ,rep_mu  = list(interp_yearly_params(year, 'rep_mu',  rep_mu))
             ,rep_sd  = list(interp_yearly_params(year, 'rep_sd',  rep_sd))
             ,min_sz  = list(interp_yearly_params(year, 'min_claim_size', min_claim_size))
        ) %>%
    mutate(data = pmap(list(pd_mu, pd_sd, pd_rt, bi_mu, bi_sd, bi_rt
                           ,bi_prop, bi_only, rate_mu, rate_sd, rep_mu, rep_sd, min_sz)
                      ,bind_rows)) %>%
...
generate_claim_data <- function(n_claims, bi_prop, bi_only, bi_mu, bi_rt, bi_sd, pd_mu, pd_rt, pd_sd, rep_mu, rep_sd) {
    
    day_offset <- sample(1:365, n_claims, replace = TRUE)

    rep_delay  <- rlnorm(n_claims, rep_mu, rep_sd) %>% as.integer
...
claim_data_tbl <- claim_simtable_tbl %>%
    mutate(size_data = pmap(list(n_claims = claim_count
                           ...
                           ,generate_claim_data)) %>%
    select(country_code, year, min_claim_size, size_data) %>%
    unnest %>%
    filter(total_amount >= min_claim_size) %>%
    mutate(incident_date = as.Date(paste0(year, '-01-01')) + day_offset
          ,report_date   = incident_date + rep_delay
    ...
    ungroup() %>%
    select(country_code, year, incident_date, report_date, claim_id, pd_amount, bi_amount, total_amount)
```
...
Having calculated the ultimate severities of all the claims, we now want to
generate the reporting of those claims in a realistic manner.

Claims develop over time, and we want to produce a development path for each
claim so that we have a more realistic set of data where we do not have
complete information about the liabilities at a given point in time.

We also have a mechanism whereby BI claims have a longer development time than
property damage (PD).
...
pd_trans_tbl <- claim_dev_data_tbl %>%
    select(country_code, year, claim_id, incident_date, report_date, pd_dev) %>%
    unnest %>%
    mutate(claim_type = 'PD')

bi_trans_tbl <- claim_dev_data_tbl %>%
    select(country_code, year, claim_id, incident_date, report_date, bi_dev) %>%
    unnest %>%
...
claim_transactions_tbl <- list(pd_trans_tbl, bi_trans_tbl) %>%
    ...
    select(country_code, year, claim_id, incident_date, report_date, transaction_date, claim_type, amount)
```
